	...
	if(val==0) {
		...
		if(d=="LEFT") {
			if(side + leftsides[val][mode] >0) {
				...
				for(let i=0;i<4;i++) {
					for(let j=0;j<4;j++) {
						if(line[i][j]==1 && board[height-4+i][side+j-1]==1) {
							l=1;
							break;
						}
					}
			...
			for(let i=0;i<4;i++) {
				for(let j=0;j<4;j++)
					if(line[i][j]==1 && (height-4+i>19 || side+j<0 || side+j>18 || board[height-4+i][side+j]==1)) {
						l=1;
						break;
					}
				if(l==1)
		...
		if(height - 4 + heights[val][mode] == 20)
			flag=1;
		else
			for(let i=0; i<4; i++) {
				for(let j=0; j<4; j++) {
					if(line[i][j]==1 && board[height-3+i][side+j]==1) {
						flag=1;
						break;
					}
				}
		...
		if(d=="LEFT") {
			if(side + leftsides[val] >0) {
				...
				for(let i=0;i<3;i++) {
					for(let j=0;j<3;j++) {
						if(shape[i][j]==1 && board[height-3+i][side+j-1]==1) {
							l=1;
							break;
						}
					}
		...
		if(height - 3 + heights[val] == 20)
			flag=1;
		else
			for(let i=0; i<3; i++) {
				for(let j=0; j<3; j++) {
					if(shape[i][j]==1 && board[height-2+i][side+j]==1) {
						flag=1;
						break;
					}
				}
		...
		if(d=="LEFT") {
			...
			for(let i=0;i<3;i++) {
				for(let j=0;j<3;j++)
					if(shape[i][j]==1 && (height-3+i>19 || side+j<0 || side+j>18 || board[height-3+i][side+j]==1)) {
						l=1;
						break;
					}
				if(l==1)
		...
		if(height - 3 + heights[val][mode] == 20)
			flag=1;
		else
			for(let i=0; i<3; i++) {
				for(let j=0; j<3; j++) {
